...
{
    _front_shininess = _back_shininess = shininess;
}

// homework
GLvoid Material::SetTransparency(GLfloat alpha)
{
    _front_ambient.SetTransparency(alpha);
    _back_ambient.SetTransparency(alpha);
    _front_diffuse.SetTransparency(alpha);
    _back_diffuse.SetTransparency(alpha);
    _front_specular.SetTransparency(alpha);
    _back_specular.SetTransparency(alpha);
    _front_emissive.SetTransparency(alpha);
    _back_emissive.SetTransparency(alpha);
}
...
GLvoid Material::ApplyDefault()
{
    GLfloat test[] = {0.2, 0.2, 0.2, 1.0};
    glMaterialfv(GL_FRONT, GL_AMBIENT, test);
    glMaterialfv(GL_BACK, GL_AMBIENT, test);

    test[0] = test[1] = test[2] = 0.8;
    glMaterialfv(GL_FRONT, GL_DIFFUSE, test);
    glMaterialfv(GL_BACK, GL_DIFFUSE, test);

    test[0] = test[1] = test[2] = 0.0;
    glMaterialfv(GL_FRONT, GL_SPECULAR, test);
    glMaterialfv(GL_BACK, GL_SPECULAR, test);
    glMaterialfv(GL_FRONT, GL_EMISSION, test);
    glMaterialfv(GL_BACK, GL_EMISSION, test);
...
{
    return _front_ambient.a() < 1 && _back_ambient.a() < 1 &&
            _front_diffuse.a() < 1 && _back_diffuse.a()  < 1 &&
            _front_specular.a() < 1 && _back_specular.a() < 1 &&
            _front_emissive.a() < 1 && _back_emissive.a() < 1;
}
