...
GLvoid TriangulatedMesh3::RenderNormals(GLfloat scale)
{
    glBegin(GL_LINES);
...
{
    lhs << rhs.VertexCount() << " " << rhs.FaceCount() << " " << rhs._leftmost_vertex << " " << rhs._rightmost_vertex << endl;

    for(GLuint i = 0; i < rhs.VertexCount(); i++){
        lhs << rhs._vertex[i] << endl;
    }
    lhs << endl;

    for(GLuint i = 0; i < rhs.VertexCount(); i++){
        lhs << rhs._normal[i] << endl;
    }
    lhs << endl;
    ...
    for(GLuint i = 0; i < rhs.FaceCount(); i++){
        lhs << rhs._face[i] << endl;
    }
    lhs << endl;
...
istream& cagd::operator >>(std::istream& lhs, TriangulatedMesh3& rhs)
{
    ...
    GLuint vertex_count = 0, face_count = 0;
    lhs >> vertex_count >> face_count >> rhs._leftmost_vertex >> rhs._rightmost_vertex;
